...
export default function LeaguePanel({ isPremium = false, leagueId: propLeagueId }: Props) {
  ...
  useEffect(() => {
    return (
      ...
      {/* Tab Content */}
      <div className="bg-[#030913] rounded-[35px] p-8 border border-white/5 shadow-inner relative overflow-hidden group min-h-[300px]">
        {tab === "standings" && <LeagueTable />}
        {tab === "fixtures" && <LeagueFixtures />}
        {tab === "topScores" && <LeagueTopScores />}
        {tab === "deepestRunners" && <LeagueDeepestRunners />}
      </div>
